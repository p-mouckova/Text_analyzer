...
if NUMBER < 1 or NUMBER > 3:
    print('Incorrect NUMBER! Please select number 1,2 or 3.')
    exit()

TEXT = str(TEXTS[NUMBER-1])
words_with_signs = TEXT.split()
words = []
while words_with_signs:
    word = words_with_signs.pop()
    word = word.strip('.,:')
    words.append(word)
...
i = 0
while i < len(counts_letters):
    word = counts_letters[i]
    if word in word_lenghts:
        frequency = word_lenghts.get(word)
